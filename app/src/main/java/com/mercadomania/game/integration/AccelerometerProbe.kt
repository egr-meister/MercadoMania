...
/**
 * Samples the accelerometer once at launch for sub13: a reading at start and one about a
 * second later.
 */
object AccelerometerProbe {
    ...
    fun start(context: Context) {
        ...
        runCatching { beginSampling(context) }.onFailure {
            ...
            // This runs inside Application.onCreate, so an unfenced throw from a vendor
            // ROM would crash the app at launch - and would leave the latch closed, so
            // every later await() would pay the full timeout.
            ready.countDown()
        }
    }
    ...
    private fun beginSampling(context: Context) {
        ...
        // Assigned only after a successful registration, so a refused probe does not
        // hold a SensorManager for the life of the process with nothing attached.
    ...
    private fun stop() {
        ...
        // The sensor must not stay live for the life of the app - it costs battery and
        // the reading is only needed once.
        sensorManager = null
        ready.countDown()
    }
}
