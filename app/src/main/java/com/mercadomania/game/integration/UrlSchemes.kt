...
/**
 * Decides which URLs belong inside the offer WebView and which must be handed to the
 * system. Offer pages link out to payment apps, messengers and `intent://` handlers;
 * those must leave the WebView, while everything the page needs to render must stay in it.
 *
...
object UrlSchemes {
     ...
     * digits, `+`, `-` or `.`. Checking that - rather than just looking for a colon -
     * is what keeps "/path:with:colons" and "2go:" from being mistaken for schemes.
     */
